...
def _run_sample(title: str, content: str) -> None:
    print(f"\n=== {title} ===")
    model = pick_model(f"{title}\n{content}")
    print(f"Model: {model}")
    try:
        summary = summarize_article(title=title, content=content, max_bullets=4, timeout_s=180, content_chars=4000)
        print(summary)
...
if __name__ == "__main__":
    ...
    print(f"EN route: {pick_model(en_text)}")
    print(f"ZH route: {pick_model(zh_text)}")

    _run_sample("US inflation cools as energy prices decline", en_text)
    _run_sample("\u4e2d\u56fd\u5236\u9020\u4e1aPMI\u56de\u5347\uff0c\u51fa\u53e3\u8ba2\u5355\u6539\u5584", zh_text)
